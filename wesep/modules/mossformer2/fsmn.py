"""Feedforward Sequential Memory Network (FSMN) modules for MossFormer2.
...
class UniDeepFsmn(nn.Module):
    """Uni-directional Deep FSMN layer."""
...
class DilatedDenseNet(nn.Module):
    """Dilated dense network for multi-scale temporal modeling."""
    ...
    def __init__(self, depth=4, lorder=20, in_channels=64):
        ...
        for i in range(self.depth):
            ...
            setattr(self, 'conv{}'.format(i + 1),
                    nn.Conv2d(
                        self.in_channels * (i + 1), self.in_channels,
...
class UniDeepFsmn_dilated(nn.Module):
    """Uni-directional Deep FSMN with dilated dense convolutions."""
